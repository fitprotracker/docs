...
| `previousSubGroupName` | `string` or `null` | Human-readable label for `previousSubGroupId`. `null` when the ID is null. **Recommended**: route by name across locations — sub-group IDs are location-scoped. |
| `newSubGroupName` | `string` or `null` | Human-readable label for `newSubGroupId`. `null` when the ID is null. |
