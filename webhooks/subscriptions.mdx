...
| **Status** | `active`, `paused`, `disabled`. See [delivery semantics](/webhooks/delivery) for when these flip automatically. |
...
- A subscription that fails for 24 hours straight automatically transitions to **disabled** — see [delivery semantics](/webhooks/delivery#retries-and-backoff). Re-enable from the same admin page.
